...
## 180 度反轉回流補修
- [x] 步驟 5：定位連續續建時反向線段可被提交的根因。
- [x] 步驟 6：在 preview 與 submit 共用方向判定，禁止端點延伸直接往來源線段 180 度回頭。
- [x] 步驟 7：執行語法檢查與 `npm run finalize`。

## 蛇形連續路徑跨行跳動補修
- [x] 步驟 8：定位左側平行蛇形物流線物品跨欄跳動的路徑資料來源。
- [x] 步驟 9：修正派貨路徑組裝，優先使用空間連續排序並保留完整段內 routePoints。
- [x] 步驟 10：執行語法檢查與 `npm run finalize`。

## 物流線不合法虛影紅色保留改造
- [x] 步驟 11：定位 invalid preview 被清空與 submit 取消拖曳的流程。
- [x] 步驟 12：讓反向或撞障礙路徑保留 raw preview ghost 並以紅色顯示。
- [x] 步驟 13：讓 invalid 左鍵提交只阻擋建造，不取消虛影拖曳狀態。
- [x] 步驟 14：執行語法檢查與 `npm run finalize`。

## 轉角顯示與鄰格誤連接補修
- [x] 步驟 15：定位轉角旁相鄰格被誤推導成轉角的渲染 fallback。
- [x] 步驟 16：移除非拓撲相接的鄰格轉角推導，避免轉彎處出現異常線段。
- [x] 步驟 17：收斂終點命中容差，禁止只在旁邊一格就與轉角物流線連接。
- [x] 步驟 18：執行語法檢查與 `npm run finalize`。

## 跨線驗證與單段編號顯示補修
- [x] 步驟 19：使用 Router footprint 統計 preview 路徑跨越的既有物流線段數。
- [x] 步驟 20：跨越 2 個以上非來源物流線段時，preview 標紅且 submit 無法建造。
- [x] 步驟 21：單擊物流線只顯示被選取線段編號；只有群組全選才顯示全部編號。
- [x] 步驟 22：執行語法檢查與 `npm run finalize`。
...
# 2026-06-26 物流線建造游標、編號開關與刪除刷子改造

## 核心目標
1. 單格物流線選取時，若開啟編號顯示，必須顯示該格在群組中的全局序號，不可從 0 重新編號。
2. 物流線編號預設關閉，僅由系統設定開關決定是否顯示。
3. 物流線建造快捷鍵激活時顯示綠色 1x1 游標；既有物流線延伸改為 Ctrl+左鍵啟動。
4. 新增底部物流線刪除工具，紅色方框跟隨滑鼠，左鍵拖曳刪除覆蓋範圍內物流線，尺寸 1x1 至 5x5。

## 實施步驟
- [x] 步驟 1：新增狀態預設與設定面板物流線編號開關。
- [x] 步驟 2：修正單格編號顯示的群組全局偏移。
- [x] 步驟 3：調整物流線建造輸入條件與建造/刪除游標 UI。
- [x] 步驟 4：執行語法檢查與 `npm run finalize`。
