...
export class MainScene extends Phaser.Scene {
    ...
    generateTextures() {
        ...
        const wrapDraw = (type) => (g, w, h) => {
            ...
            // 修正：手動偏移才是最穩定的
            BuildingRenderer.drawEntity(this, g, { type1: type, isUnderConstruction: false }, 1.0);
            g.restore();
        };
        ...
        buildingTypes.forEach(type => {
            ...
            for (let rotationSteps = 1; rotationSteps < 4; rotationSteps++) {
                createTex(`tex_${type}_r${rotationSteps}`, uw * TS + 20, uh * TS + 40, (g, w, h) => {
                    ...
                    BuildingRenderer.drawEntity(this, g, { type1: type, isUnderConstruction: false, rotationSteps }, 1.0);
                    g.restore();
                });
            }
        });
    ...
    updateDynamicHUD(visibleEntities) {
        ...
        visibleEntities.forEach(ent => {
            ...
            if (ent.isUnderConstruction) {
                HUDRenderer.drawBuildProgressBar(this, g, ent, uw, uh, TS);
                // 施工中時，確保生產隊列標籤隱藏
                ...
                HUDRenderer.drawUpgradeProgressBar(this, g, ent, uw, uh, TS);
            } else if (ent.queue) {
                // 不論 queue.length 是否大於 0 都呼叫，讓 drawProductionHUD 內部處理隱藏邏輯
                HUDRenderer.drawProductionHUD(this, g, ent, uw, uh, TS);
            }
            ...
            if (ent.rallyPoint && isSelected) {
                HUDRenderer.drawRallyPoint(g, ent);
            }
            ...
            if (canShowWorkerOccupancy) {
                ...
                HUDRenderer.drawWorkerLights(this, occG, ent, ent.x, ent.y, uw, uh, TS, 1.0);
        ...
        for (let i = 0; i < visibleEntities.length; i++) {
            ...
            if (!displayObj) {
                ...
                if (textureKey && this.textures.exists(textureKey)) {
                    ...
                    BuildingRenderer.drawEntity(this, displayObj, ent, 1.0);
                    this.entities.set(id, displayObj);
                    this.entityGroup.add(displayObj);
                    displayObj.setDepth(500000 + ent.y); // Requirement 3: Y-axis sorting + Huge Offset
                }
    ...
    updatePlacementPreview(state) {
        ...
        if (state.previewPos && (state.buildingMode === 'DRAG' || state.buildingMode === 'STAMP' || state.buildingMode === 'NONE')) {
            ...
            BuildingRenderer.drawEntity(this, g, {
