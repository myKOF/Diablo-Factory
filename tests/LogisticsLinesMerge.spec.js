...
test('Recorded Logical E2E Test', async ({ page }) => {
    ...
    await executeLogic(page, () => {
        ...
        if (workers.length >= 3) {
            ...
            window.GameEngine.workerSystem.handleWorkerCommand(workers[0], storehouse);
            window.GameEngine.workerSystem.handleWorkerCommand(workers[1], storehouse);
            window.GameEngine.workerSystem.handleWorkerCommand(workers[2], storehouse);
        } else {
    ...
    await executeLogic(page, () => {
        const tc = window.GameEngine.state.mapEntities.find(e => e.type1 === 'village');
        const storehouse = window.GameEngine.state.mapEntities.find(e => e.id === 'core_storehouse');
        if (tc && storehouse) {
            tc.rallyPoint = { x: storehouse.x, y: storehouse.y, targetId: storehouse.id, targetType: 'building', name: storehouse.name };
            console.log("[DEBUG] 集結點已設定至倉庫");
        } else {
            console.error("[DEBUG] 找不到城鎮中心！", tc, storehouse);
        }
    });
    // 全局生產 RANDOM (指定城鎮中心作為生產建築)
    await executeLogic(page, () => { const b = window.GameEngine.state.mapEntities.find(x => x.id === 'core_village'); if (b) window.GameEngine.addToProductionQueue(null, 'RANDOM', b); });
    ...
    // 設定三條物流線各自的過濾器 (按照用戶操作順序)
    await executeLogic(page, () => {
        const storehouse = window.GameEngine.state.mapEntities.find(e => e.id === 'core_storehouse');
        if (!storehouse || !storehouse.outputTargets) { console.error('[DEBUG] 找不到倉庫 outputTargets'); return; }
        console.log('[DEBUG] outputTargets count:', storehouse.outputTargets.length);
        storehouse.outputTargets.forEach((conn, i) => console.log(`  [${i}] lineId=${conn.lineId} filter=${conn.filter}`));
        // 設第一條 (木頭)、第二條 (石頭)、第三條 (金礦石)
        if (storehouse.outputTargets[0]) storehouse.outputTargets[0].filter = 'wood';
        if (storehouse.outputTargets[1]) storehouse.outputTargets[1].filter = 'stone';
        if (storehouse.outputTargets[2]) storehouse.outputTargets[2].filter = 'gold_ore';
    });
    // [系統日誌] [物流] 路線搬運品項已更新：石頭。
    // [系統日誌] [物流] 路線搬運品項已更新：金礦石。
    // 拖曳建造物流線 (GroupId: new) - 最後第四條線
    ...
    await page.waitForTimeout(1000);
    await page.screenshot({ path: 'tmp/test-results/screenshot_' + Date.now() + '.png' });
});
